...
print(f"out: {out}, out.shape: {np.shape(out)}") # out: [[2.74279033 2.64105041]], out.shape: (1, 2)

out = affine2.forward(out) # 두번째 레이어 예측값 y^2_1
print(f"out: {out}, out.shape: {np.shape(out)}") # out: [[2.54127985]], out.shape: (1, 1)

out = mse.forward(out, t) # 손실값 Loss
print(f"out: {out}, out.shape: {np.shape(out)}") # out: 3.229051646341373, out.shape: ()

# 역전파
dout = np.ones(1,) # dL/dL
dout = mse.backward(dout) # dL/dy^2_1
print(f"dout: {dout}, dout.shape: {np.shape(dout)}") # dout: [[2.54127985]], dout.shape: (1, 1)

dout = affine2.backward(dout) # dL/dy^1_1: [[dL/dy^1_1], [dL/dy^1_2]]
print(f"dout: {dout}, dout.shape: {np.shape(dout)}") # dout: [[4.74867999 2.41558857]], dout.shape: (1, 2)

print(f"affine2.dW: {affine2.dW}, affine2.dW.shape: {np.shape(affine2.dW)}") # affine2.dW: [[6.97019781] [6.71164819]], affine2.dW.shape: (2, 1)
print(f"affine2.db: {affine2.db}, affine2.db.shape: {np.shape(affine2.db)}") # affine2.db: [2.54127985], affine2.db.shape: (1,)

dout - affine1.backward(dout) # dL/dx: [[dL/dx^1_1], [dL/dx^1_2]]
print(f"dout: {dout}, dout.shape: {np.shape(dout)}") # dout: [[ 4.7459875  -2.48353659]], dout.shape: (1, 2)

print(f"affine1.dW: {affine1.dW}, affine1.dW.shape: {np.shape(affine1.dW)}") # affine1.dW: [[ 4.7459875  -2.48353659] [ 4.7459875  -2.48353659]], affine1.dW.shape: (2, 2)
print(f"affine1.db: {affine1.db}, affine1.db.shape: {np.shape(affine1.db)}") # affine1.db: [ 4.7459875  -2.48353659], affine1.db.shape: (2,)