...
class MultiLayerNet:
    ...
    def gradient(self, x, t):
        ...
        # backward
        dout = 1
        dout = self.last_layer.backward(dout) # (4,1)
        # print(f"Last layer dout: {dout}, shape: {np.shape(dout)}")

        layers = list(self.layers.values())
        layers.reverse()
        # print(f"layers.reverse: {layers}")
        for layer in layers:
            dout = layer.backward(dout)
            # print(layer)
            # print(f"dout: {dout}, shape: {np.shape(dout)}")
