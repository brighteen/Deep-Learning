...
tracked_objects = {}
object_id_counter = 0
stillness_duration_threshold = 50  # 움직임 없음 지속 시간 (초)
...
while cap.isOpened():
    ...
    current_classes = results[0].boxes.cls.cpu().numpy().astype(int) # 클래스 ID 가져오기
    ...
    for obj_id, (prev_bbox, stillness_duration, initial_center, prev_class_id) in list(tracked_objects.items()): # 이전 클래스 ID 받기
        ...
        if best_match_index != -1:
            ...
            current_class_id = current_classes[best_match_index] # 현재 클래스 ID 가져오기
