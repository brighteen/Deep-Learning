...
# 클래스 이름 매핑 (학습 시 레이블 순서와 일치해야 함)
class_names = {0: "chicken", 1: "rice"}
rice_color = (128, 0, 128) # 보라색
...
def calculate_iou(box1, box2):
    ...
    intersection_area = (x_right - x_left) * (y2_1 - y1_1)
...
while cap.isOpened():
    ...
    current_detections_all = results[0].boxes.xyxy.cpu().numpy().astype(int)
    current_classes_all = results[0].boxes.cls.cpu().numpy().astype(int) # 클래스 ID 가져오기
    ...
    for i, class_id in enumerate(current_classes_all):
        bbox = current_detections_all[i]
        if class_id == 1: # rice
            rice_indices.append(i)
            cv2.rectangle(original_frame, (bbox[0], bbox[1]), (bbox[2], bbox[3]), rice_color, 2)
            cv2.putText(original_frame, class_names[class_id], (bbox[0], bbox[1] - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.5, rice_color, 2)
        elif class_id == 0: # chicken
            current_detections.append(bbox)
            current_classes.append(class_id)
            chicken_sizes.append((bbox[2] - bbox[0]) * (bbox[3] - bbox[1])) # 박스 면적

    # 닭 크기 기반 필터링
    if chicken_sizes:
        median_size = np.median(chicken_sizes)
        min_size_threshold = median_size * 0.5 # 예시: 중간 크기의 50% 이상
        max_size_threshold = median_size * 1.5 # 예시: 중간 크기의 150% 이하
        ...
        for i, bbox in enumerate(current_detections):
            ...
            if min_size_threshold <= size <= max_size_threshold:
                filtered_detections.append(bbox)
                filtered_classes.append(current_classes[i])
                filtered_indices.append(i)

        current_detections = filtered_detections
        current_classes = filtered_classes

    new_tracked_objects = {}
    current_detected_indices = list(range(len(current_detections)))
    matched_indices = set()

    for obj_id, (prev_bbox, stillness_duration, initial_center, prev_class_id) in list(tracked_objects.items()): # 이전 클래스 ID 받기
        best_match_index = -1
        max_iou = 0
        ...
        if best_match_index != -1:
            ...
            current_class_index = list(range(len(current_detections))).index(best_match_index) # 현재 filtered detection에서의 인덱스
            current_class_id = current_classes[current_class_index] # 현재 클래스 ID 가져오기
            ...
            if is_still_by_center and is_still_by_optical_flow:
                new_tracked_objects[obj_id] = [current_bbox, stillness_duration + time_elapsed, initial_center, current_class_id] # 클래스 ID 저장
            else:
                # 움직임 감지: 초기 중심점 업데이트 및 시간 초기화
                new_tracked_objects[obj_id] = [current_bbox, 0.0, current_center, current_class_id] # 클래스 ID 저장
            ...
            new_tracked_objects[obj_id] = [prev_bbox, stillness_duration + time_elapsed, initial_center, prev_class_id] # 이전 클래스 ID 유지

    # 새로 감지된 객체 추가 (닭만 해당)
    for index in current_detected_indices:
        bbox = current_detections[index]
        class_id = current_classes[index]
        if class_id == 0: # chicken
            center = ((bbox[0] + bbox[2]) // 2, (bbox[1] + bbox[3]) // 2)
            new_tracked_objects[object_id_counter] = [bbox, 0.0, center, class_id] # 클래스 ID 저장
            object_id_counter += 1
    ...
    dead_count = 0
    center_counts = {"green": 0, "yellow": 0, "blue": 0}

    for obj_id, (bbox, stillness_duration, initial_center, class_id) in tracked_objects.items(): # 클래스 ID 가져오기
        x1, y1, x2, y2 = bbox

        if class_id == 0: # chicken
            color = (0, 255, 0)  # 기본은 초록색 (움직이는 상태)
            label = f"{class_names.get(class_id, 'Unknown')} {obj_id}"
            ...
            if stillness_duration > 0:
                ...
                if color == (0, 255, 0):
                    center_counts["green"] += 1
                elif color == (0, 255, 255):
                    center_counts["yellow"] += 1
                elif color == (0, 0, 255):
                    center_counts["blue"] += 1
        # rice는 여기에서 처리하지 않음

    # Center Points 창에 Grid 추가 및 개수 표시
    height, width, _ = center_points_frame.shape
    grid_spacing = 50
    for i in range(0, height, grid_spacing):
        cv2.line(center_points_frame, (0, i), (width, i), (100, 100, 100), 1)
    for j in range(0, width, grid_spacing):
        cv2.line(center_points_frame, (j, 0), (j, height), (100, 100, 100), 1)

    cv2.putText(center_points_frame, f"Green: {center_counts['green']}", (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 0), 2)
    cv2.putText(center_points_frame, f"Yellow: {center_counts['yellow']}", (10, 60), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 255), 2)
    cv2.putText(center_points_frame, f"Blue: {center_counts['blue']}", (10, 90), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 255), 2)

    # Dead Chickens 창에 Grid 추가 및 개수 표시
    height_dead, width_dead, _ = dead_chicken_frame.shape
    for i in range(0, height_dead, grid_spacing):
        cv2.line(dead_chicken_frame, (0, i), (width_dead, i), (100, 100, 100), 1)
    for j in range(0, width_dead, grid_spacing):
        cv2.line(dead_chicken_frame, (j, 0), (j, height_dead), (100, 100, 100), 1)

    cv2.putText(dead_chicken_frame, f"Dead Chickens: {dead_count}", (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 255), 2)
