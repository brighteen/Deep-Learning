...
def is_dead_chicken(region_img):
    ...
    with torch.no_grad():
        ...
        print("[debug]", torch.softmax(output, dim=1))
    # 소프트맥스 적용 후, 특정 클래스(예: index 1: 닭)의 확률이 임계값(예: 0.5) 이상이면 True 반환
    prob = torch.softmax(output, dim=1)
    print("[debug] Classification prob:", prob)
    if prob[0, 1] > 0.5:
        return True
    return False
# (2) 영상에서 정지 객체 검출 및 후보 영역 추출
file_path = "Object_Detection/객체탐지 실습/datas/tile_r0_c0.mp4"  # 분석할 영상 파일 경로
# file_path = "Object_Detection/객체탐지 실습/datas/0_8_IPC1_20221105100432.mp4"  # 분석할 영상 파일 경로
