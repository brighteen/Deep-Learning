...
# 결과 저장할 디렉토리 설정
output_dir = r"C:\Users\brigh\Documents\GitHub\Deep-Learning\Object_Detection\0528_id집합표현개선2\results\11.집합정렬제발좀"
...
# ID 집합 관리를 위한 딕셔너리 - {대표ID: [대표ID, id1, id2, ...]} 형태로 관리
id_mappings = {}  
...
def merge_id_sets(id1, id2):
    ...
    # 새 집합 생성 - 대표 ID가 첫 번째에 오는 리스트 형태로 저장
    id_list = sorted(all_related_ids)
    id_list.remove(main_id)
    id_list.insert(0, main_id)  # 대표 ID를 첫 번째로 이동
...
# 통합된 ID 집합 정보 형식화 함수 (수정)
def format_id_set(id_set, main_id, for_display=False):
    """
    ID 집합 형식화 함수
    ...
    Args:
        id_set: ID 집합
        main_id: 대표 ID
        for_display: True면 정렬된 리스트 반환, False면 집합 반환
    
    Returns:
        for_display=True: 정렬된 리스트 (화면 표시용)
        for_display=False: 집합 (내부 저장용, 하지만 생성 시 정렬된 순서로)
    """
    # 전체를 정렬
    sorted_list = sorted(id_set)
    
    # 대표 ID를 첫 번째로 이동 (대표 ID가 이미 가장 작은 값이어야 함)
    ...
    # 반환 타입 결정
    if for_display:
        return sorted_list  # 화면 표시용 정렬된 리스트
    else:
        return set(sorted_list)  # 내부 저장용 집합

# ID 집합을 정렬된 형태로 표시하는 함수 추가
def get_sorted_id_mappings():
    """내부 저장용 id_mappings를 정렬된 형태로 변환"""
    sorted_mappings = {}
    for main_id, id_set in id_mappings.items():
        sorted_mappings[main_id] = format_id_set(id_set, main_id, for_display=True)
...
for frame_idx in range(start_frame, end_frame, frame_interval):
            ...
            for i, (box, id) in enumerate(zip(boxes.xyxy.cpu().numpy(), boxes.id.int().cpu().numpy())):
                ...
                if id not in previous_ids:
                    ...
                    if matched_id:
                        ...
                        # ID 집합 정보를 텍스트로 표시 (정렬된 형태)
                        if len(id_set) > 1:  # 집합에 여러 ID가 있는 경우
                            display_list = format_id_set(id_set, main_id, for_display=True)
                            text = f"ID: {main_id}:{display_list}"
    ...
    for disappeared_id in newly_disappeared:
        if disappeared_id in last_positions:
            ...
            if len(id_set) > 1:  # 집합에 여러 ID가 있는 경우
                display_list = format_id_set(id_set, main_id, for_display=True)
                text = f"ID: {main_id}:{display_list} (Lost)"
            else:  # 집합에 하나의 ID만 있는 경우
