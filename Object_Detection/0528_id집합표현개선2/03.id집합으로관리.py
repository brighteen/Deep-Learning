...
# ID 집합 정보 형식화 함수 - 대표 ID가 항상 첫번째에 오도록 설정
def format_id_set(id_set, main_id):
    # 집합을 리스트로 변환하여 순서 조정
    id_list = list(id_set)
    if main_id in id_list:
        id_list.remove(main_id)
        id_list.insert(0, main_id)  # 대표 ID를 첫번째로 이동
    return set(id_list)  # 다시 집합으로 변환
